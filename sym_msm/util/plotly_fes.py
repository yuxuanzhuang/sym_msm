...
def export_plotly(tica_csv, title, output, sel_tic=[0, 1], append=False):
    ...
    z, xedge, yedge = np.histogram2d(tic_values[0], tic_values[1], bins=50, weights=weights)
    ...
    if True:
        fig.add_trace(
            go.Contour(x=x, y=y, z=f, zmax=10, zmin=0, zmid=3, ncontours=10, colorscale="Earth", showscale=False)
        )
    ...
    for system, df in plotly_df.groupby("system"):
        ...
        fig.add_trace(
            go.Scattergl(
                ...
                marker=dict(color=t, colorscale="Purp", size=weights_norm * 10, opacity=1, showscale=False),
            )
        )
...
def export_plotly_3d(
    ...
    fig.add_trace(
        go.Scatter3d(
            ...
            mode="markers",
            showlegend=False,
            marker=dict(color="black", size=3, opacity=0.4, showscale=False),
        )
    )
...
def main(args):
    ...
    export_plotly(tica_csv=args.tica_csv, output=args.output, title=args.title)
